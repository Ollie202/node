...
pub(crate) const LEAVES_CF: &str = "leaves";
/// The names of the `RocksDB` column families used for storing SMT subtrees (deep nodes).
pub(crate) const SUBTREE_24_CF: &str = "st24";
pub(crate) const SUBTREE_32_CF: &str = "st32";
pub(crate) const SUBTREE_40_CF: &str = "st40";
pub(crate) const SUBTREE_48_CF: &str = "st48";
pub(crate) const SUBTREE_56_CF: &str = "st56";
pub(crate) const SUBTREE_DEPTHS: [u8; 5] = [56, 48, 40, 32, 24];
...
impl RocksDbStorage {
    ...
    #[inline(always)]
    pub(crate) fn index_db_key(index: u64) -> [u8; 8] {
        index.to_be_bytes()
    }
    ...
    #[inline(always)]
    pub(crate) fn subtree_db_key(index: NodeIndex) -> KeyBytes {
    ...
    /// Creates a new [`crate::rocksdb_snapshot::RocksDbSnapshotStorage`] from this storage's
    /// database.
    pub fn snapshot_storage(&self) -> crate::rocksdb_snapshot::RocksDbSnapshotStorage {
        crate::rocksdb_snapshot::RocksDbSnapshotStorage::new(Arc::clone(&self.db))
    }
}
...
pub(crate) struct RocksDbDirectLeafIterator<'a> {
    pub(crate) iter: DBIteratorWithThreadMode<'a, DB>,
}
...
pub(crate) struct RocksDbSubtreeIterator<'a> {
    db: &'a DB,
...
impl<'a> RocksDbSubtreeIterator<'a> {
    pub(crate) fn new(db: &'a DB, cf_handles: Vec<&'a rocksdb::ColumnFamily>) -> Self {
...
#[inline(always)]
pub(crate) fn cf_for_depth(depth: u8) -> &'static str {
    match depth {
        24 => SUBTREE_24_CF,
        32 => SUBTREE_32_CF,
        40 => SUBTREE_40_CF,
